        ...
        try:
            ...
            if model is not None:
                ...
                with torch.no_grad():  # Chế độ không tính đạo hàm
                    ...
                    # Tạo cảnh báo từ kết quả detection
                    for _, row in detections.iterrows():
                        class_name = row['name']
                        confidence = row['confidence']
                        x1, y1, x2, y2 = int(row['xmin']), int(row['ymin']), int(row['xmax']), int(row['ymax'])

                        # Ví dụ: Tạo cảnh báo nếu detect được biển báo "stop" với độ tin cậy cao
                        if class_name == 'stop' and confidence > 0.7:
                            alerts.append(f"Phát hiện biển báo STOP ({confidence:.2f}) tại ({x1}, {y1}) - ({x2}, {y2})")
            ...
            return jsonify({'frame': frame_string, 'alerts': alerts})  # Truyền danh sách cảnh báo
        except Exception as e:
